...
class Critic(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed, fcs1_units=64, fc2_units=64):
        ...
        self.seed = torch.manual_seed(seed)
        # self.fcs1 = nn.Linear(out_features=fcs1_units, in_features=(state_size + action_size) * 3)
        # self.fc2 = nn.Linear(out_features=fc2_units, in_features=fcs1_units)
        # self.fc2 = nn.Linear(out_features=fcs1_units, in_features=fcs1_units+action_size)
    ...
    def forward(self, states, actions, action):
        ...
        # x = F.relu(self.fcs1(torch.cat((states, actions), dim=1)))
        # x = F.relu(self.fc2(torch.cat((x, action), dim=1)))
        xs = F.relu(self.fcs1(states))
        x = torch.cat((xs, actions), dim=1)
        # x = F.relu(self.fc2(torch.cat((x,action), dim=1)))
        # x = F.relu(self.fcs1(feed_ip))  # feed_ip shape = (1024,69)
        x = F.relu(self.fc2(x))
        return self.fc3(x)
